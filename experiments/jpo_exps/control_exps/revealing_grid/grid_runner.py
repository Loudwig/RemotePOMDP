"""Shared resumable runner for the control revealing grids."""
...
ROOT = (
    ...
    / "jpo_grid_s6_seed1111_g09_b10_e10_eps003_030_t700_fib"
)

EXPERIMENT = {
    "name": "jpo_grid_s6_seed1111_g09_b10_e10_eps003_030_t700_fib",
    "comparison_experiment": "../jpo_grid_s6_seed1111_g09_b10_e10_eps003_030_t700",
    "n_states": 6,
    "n_actions": 2,
    "density": 0.5,
    "reward_decay": 10.0,
    ...
    try:
        ...
        summary = {
            ...
            **point,
            ...
            "discounted_transmission_occupancy": _discounted_transmission_occupancy(
                model, analysis
            ),
        ...
        if not settings["export_beliefs"]:
            _write_json(
                jpo_dir / "native_output" / "discarded_belief_dump.json",
                {
                    ...
                    "reason": "Belief dump disabled for the 100-point comparison grid.",
                },
            )
        return summary
...
def _write_merged(points_by_id: dict[str, dict[str, Any]]) -> None:
    _write_json(
        ROOT / "results.json",
        {
            ...
            "points": sorted(points_by_id.values(), key=lambda point: point["index"]),
        },
    )


def _priority_key(point: dict[str, Any]) -> tuple[Any, ...]:
    priority = EXPERIMENT["priority_points"]
    if point["point_id"] in priority:
        return (0, priority.index(point["point_id"]))
    return (1, -point["beta"], point["epsilon"])
...
def main() -> None:
    ...
    if results_path.is_file():
        ...
        existing = {point["point_id"]: point for point in payload.get("points", [])}
    ...
    print("next=" + ",".join(point["point_id"] for point in pending[:8]), flush=True)
    if args.dry_run:
        ...
        for future in as_completed(future_to_point):
            ...
            if summary["status"] == "ok":
                print(
                    ...
                    f"b={point['beta']:.6f} e={point['epsilon']:.6f} "
